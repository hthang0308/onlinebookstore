...
const CourseList = (props) => {
  ...
  const submitHandler = (event) => {
    ...
    setIsLoading(true);
    get("/api/book/search?q=" + enteredSearchText)
      .then((res) => {
        setDataContent(res.data.content.items);
      })
      .then(() => setIsLoading(false));
  ...
  return (
    <>
      <form className="form-inline m-2 ml-4 mt-4 mr-2" onSubmit={submitHandler}>
        <input className="form-control" type="search" placeholder="Search Here..." aria-label="Tìm kiếm" onChange={enteredSearchTextChangeHandler} />
        <button className="btn btn-outline-success my-2 ml-2 my-sm-0" type="submit">
          Search
        </button>
      </form>
      ...
      <div>
        ...
        {!isLoading && dataContent.length === 0 && <div className="ml-2">No course!</div>}
